...
describe("Event related Notification test cases: ", function() {
  this.timeout(30000)
  var firstUserObj = null
  var eventObj = null
  it("create a new event: ", function(done) {
    utils.async.waterfall([
        function loginFirstUser(callback) {
          ...
          service.eventService.createEvent(firstUserObj, data, callback)
        },function getNotificationTrigger(event,callback){
          eventObj = event
          validateSuccessfulEventCreate(event)
        ...
        },function testLaunchUpComingReminders(notifTrigger, callback) {
          models.event.model.update({"_id":eventObj._id},{"$set":{'launchDate':utils.moment().utc().add(-10, "minutes")}},function(err,data){
  ...
  after(function() {
    utils.l.d("Removing user", firstUserObj)
    models.user.model.remove({_id: {$in: [firstUserObj._id]}}, function(err, data) {})
    utils.l.d("Removing user:2222")
    models.userGroup.model.remove({"user": {$in: [firstUserObj._id]}}, function(err, data) {})
    utils.l.d("Removing user:3333")
    models.eventInvitation.model.remove({"inviter": {$in: [firstUserObj._id]}}, function(err, data) {})
    utils.l.d("Removing user:2222")
    models.eventInvitation.model.remove({"invitee": {$in: [firstUserObj._id]}}, function(err, data) {})
    models.event.model.remove({"players": {$in: [firstUserObj._id]}}, function(err, data) {})
    models.notificationQueue.model.remove({"eventId": {$in: [eventObj._id]}}, function(err, data) {})
  })
})
